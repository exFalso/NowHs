...
embedNoError :: (EmbedSwitch err, Monad m) => NowHsT False m a -> NowHsT err m a
embedNoError (NowHsT m) = NowHsT (embedSwitchT m)

runNowHsTNoError :: (MonadIO m) => NowHsT False m a -> m a
